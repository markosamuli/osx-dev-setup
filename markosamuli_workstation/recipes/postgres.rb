...
include_recipe "markosamuli_workstation::increase_shared_memory"

run_unless_marker_file_exists("postgres") do

  ["homebrew.mxcl.postgresql.plist", "org.postgresql.postgres.plist" ].each do |plist|
    plist_path = File.expand_path(plist, File.join(node['sprout']['home'], 'Library', 'LaunchAgents'))
...
# blow away default image's data directory
  ...
  launch_agents_path = File.expand_path('.', File.join(node['sprout']['home'], 'Library', 'LaunchAgents'))
  ...
  execute "copy over the plist" do
    command %'cp /usr/local/Cellar/postgresql/9.*/homebrew.mxcl.postgresql.plist #{node['sprout']['home']}/Library/LaunchAgents/'
    user node['current_user']
  end

  execute "start the daemon" do
    command %'launchctl load -w #{node['sprout']['home']}/Library/LaunchAgents/homebrew.mxcl.postgresql.plist'
    user node['current_user']
  end
